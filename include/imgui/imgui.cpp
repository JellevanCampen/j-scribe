...
void ImGui::RenderTextColor(ImVec2 pos, const char* text, const char* text_end, const ImVec4& color, bool hide_text_after_hash)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
void ImGui::RenderTextWrappedColor(ImVec2 pos, const char* text, const char* text_end, float wrap_width, const ImVec4& color)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  ...
  if (text_len > 0)
  {
    window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(color), text, text_end, wrap_width);
    if (g.LogEnabled)
      LogRenderedText(&pos, text, text_end);
  }
}
...
void ImGui::RenderTextClippedColor(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect, const ImVec4& color)
{
  // Hide anything after a '##' string
  const char* text_display_end = FindRenderedTextEnd(text, text_end);
  const int text_len = (int)(text_display_end - text);
  ...
  {
    ImVec4 fine_clip_rect(clip_min->x, clip_min->y, clip_max->x, clip_max->y);
    window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(color), text, text_display_end, 0.0f, &fine_clip_rect);
  }
  else
  {
    window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(color), text, text_display_end, 0.0f, NULL);
  }
  if (g.LogEnabled)
    LogRenderedText(&pos, text, text_display_end);
}
...
void ImGui::RenderTriangleColor(ImVec2 p_min, ImGuiDir dir, float scale, const ImVec4& color)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  ...
  window->DrawList->AddTriangleFilled(center + a, center + b, center + c, GetColorU32(color));
}
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
			...
			// shadowSettings.rectPos = window->Pos;
			// shadowSettings.rectSize = window->Size;
			shadowSettings.sigma = 12;
			shadowSettings.shadowOffset = ImVec2(4, 4);
			shadowSettings.shadowColor = ImVec4(0.f, 0.f, 0.f, 0.5f);
			shadowSettings.padding = ImVec2(3 * shadowSettings.sigma, 3 * shadowSettings.sigma);
			shadowSettings.rectPos = ImVec2(0, 0);
			shadowSettings.rectSize = window->Size;
			shadowSettings.shadowSize = shadowSettings.rectSize + shadowSettings.padding * 2;
			shadowSettings.rings = 4;
			shadowSettings.linear = true;
			//drawRectangleShadowVerticesAdaptive(shadowSettings);
			auto shadowColor = GetColorU32(ImVec4(0.f, 0.f, 0.f, 0.05f));
			window->DrawList->AddRectFilled(window->Pos + ImVec2(8-2, 8-2), window->Pos + window->Size + ImVec2(8-2, 8-2), shadowColor, window_rounding, ImGuiCorner_All);
			window->DrawList->AddRectFilled(window->Pos + ImVec2(4-2, 4-2), window->Pos + window->Size + ImVec2(12-2, 12-2), shadowColor, window_rounding, ImGuiCorner_All);
			window->DrawList->AddRectFilled(window->Pos + ImVec2(0-2, 0-2), window->Pos + window->Size + ImVec2(16-2, 16-2), shadowColor, window_rounding, ImGuiCorner_All);
			window->DrawList->AddRectFilled(window->Pos + ImVec2(-4-2, -4-2), window->Pos + window->Size + ImVec2(20-2, 20-2), shadowColor, window_rounding, ImGuiCorner_All);
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
            ...
            {
                RenderTriangleColor(window->Pos + style.FramePadding, window->Collapsed ? ImGuiDir_Right : ImGuiDir_Down, 1.0f, ImVec4(1.f, 1.f, 1.f, 1.f));
            }
            ...
            RenderTextClippedColor(text_min, text_max, name, NULL, &text_size, style.WindowTitleAlign, &clip_rect, ImVec4(1.f, 1.f, 1.f, 1.f));
        }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (display_frame)
    {
        // Framed type
        RenderFrame(bb.Min, bb.Max, col, true, style.FrameRounding);
        RenderTriangleColor(bb.Min + ImVec2(padding.x, text_base_offset_y), is_open ? ImGuiDir_Down : ImGuiDir_Right, 1.0f, ImVec4(1.f, 1.f, 1.f, 1.f));
        if (g.LogEnabled)
        {
            ...
            RenderTextClippedColor(text_pos, bb.Max, label, label_end, &label_size, ImVec2(0.f, 0.f), 0, ImVec4(1.f, 1.f, 1.f, 1.f));
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        window->DC.CursorPos.x += (float)(int)(style.ItemSpacing.x * 0.5f);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, style.ItemSpacing * 2.0f);
        PushStyleColor(ImGuiCol_Text, GetColorU32(ImVec4(1.f, 1.f, 1.f, 1.f)));
        float w = label_size.x;
